                                  ...
                                  ucp_proto_stage_t proto_stage)
{
    ...
    status = ucp_proto_multi_init_priv(params, mpriv, priv_size_p, perf_node,
                                       &perf);
    if (status != UCS_OK) {
        return status;
    }

    return ucp_proto_common_init_caps(&params->super, &perf, perf_node,
                                      mpriv->reg_md_map, extra_steps_range,
                                      proto_stage);
}
